...
import { User, Bell, Shield, Upload, XCircle, Lock } from 'lucide-react';
...
const ProfileSettings = () => {
	...
	// Use custom hook for data and logic
	const {
		userInfo,
		loading,
		error,
	...
	// Sync editForm with userInfo when userInfo updates
	useEffect(() => {
		if (userInfo) {
			setEditForm({
	...
	const handleImageChange = (e) => {
		...
		if (file.size > maxSize) {
			setError(t('profileSettings.errors.invalidImageSize'));
			return;
		}
	...
	const handleCropComplete = async () => {
		...
		try {
			...
			setError(t('profileSettings.errors.cropImageError'));
		}
	};
	...
	const handleEmailChangeFlow = async (newEmail) => {
		clearMessages();
		const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
		if (!emailRegex.test(newEmail)) {
			setError(t('profileSettings.errors.invalidEmail'));
			return;
		}

		// Check availability via hook
		const available = await checkEmailAvailability(newEmail);
		if (!available) {
			setError(t('profileSettings.errors.emailInUse'));
			return;
		}

		// Initiate change via hook
		const initiated = await initiateEmailChange(newEmail);
		if (initiated) {
	...
	const handleVerifyEmail = async () => {
		clearMessages();
		try {
			await verifyEmailChange(verificationModal.code);
			// On success:
			...
			setVerificationModal((prev) => ({
				...prev,
				error: error.response?.data?.message || t('profileSettings.errors.invalidVerificationCode'),
			}));
		}
	};

	const handleCancelEmailChangeRequest = async () => {
	...
	const handleSaveChanges = async () => {
		...
		if (editForm.email !== userInfo.email) {
			await handleEmailChangeFlow(editForm.email);
			return;
		}
		...
		if (editForm.name !== userInfo.name) {
			if (editForm.name.length < 2 || editForm.name.length > 50) {
				setError(t('profileSettings.errors.invalidNameLength'));
				return;
			}
		}
		...
		if (!hasNameChange && !hasImageChange) {
			setError(t('profileSettings.errors.noChanges'));
			return;
		}
		...
		<SidebarLayout>
			...
			>
						...
						{[
							...
							>
								<div className='flex flex-col sm:flex-row items-center justify-center sm:space-x-2'>
									...
									<span className='text-xs sm:text-sm md:text-base mt-1 sm:mt-0'>{label}</span>
								</div>
							</motion.button>
						))}
					</motion.div>
								...
								{activeTab === 'security' && (
									<div className='space-y-6'>
										...
										<div>
											<h3
												dir='rtl'
												className='text-lg font-semibold text-gray-700 dark:text-gray-300 border-b pb-2 mb-4'
											>
												{t('profileSettings.appearanceHeader')}
											</h3>
											<div className='bg-white dark:bg-gray-700 rounded-lg p-4 sm:p-6 shadow-sm'>
                              ...
                              "
										...
										<div>
											<h3
												...
												className='text-lg font-semibold text-gray-700 dark:text-gray-300 border-b pb-2 mb-4'
											>
												{t('profileSettings.securityHeader') || 'Security'}
											</h3>
											<div className='bg-white dark:bg-gray-700 rounded-lg p-4 sm:p-6 shadow-sm flex items-center justify-between'>
												<div>
													<p className='text-sm font-medium text-gray-900 dark:text-gray-200'>
														{t('profileSettings.password') || 'Password'}
													</p>
													<p className='text-sm text-gray-500 dark:text-gray-400'>
														{t('profileSettings.changePasswordDesc') ||
															'Update your password to keep your account secure.'}
													</p>
												</div>
												<button
													onClick={() => navigate('/change-password')}
													className='flex items-center px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 focus:ring-4 focus:ring-blue-300 transition-colors'
												>
				...
				<VerificationModal
					...
					onCancelEmailChange={handleCancelEmailChangeRequest}
				/>
