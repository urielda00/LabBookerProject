...
const DashBoard = () => {
	...
	// Helper function for formatting growth (purely UI logic)
	const formatGrowth = (value) => {
		...
		const colorClass =
			numericValue > 0 ? 'text-green-600' : numericValue < 0 ? 'text-red-600' : 'text-gray-600';
					...
					>
						...
						<div className='xl:col-span-2 space-y-4 sm:space-y-6'>
								...
								{cards.map((card, index) => (
									...
									>
										...
										<div className='mt-3 pt-3 border-t border-gray-100 dark:border-gray-700'>
											<span className='text-xs sm:text-sm dark:text-gray-300'>{card.trend}</span>
										</div>
									</motion.div>
								))}
							</div>
							...
							<div className='grid grid-cols-2 md:grid-cols-2 gap-2 sm:gap-3'>
								{bookingStatusCards.map((item, index) => (
									<motion.div
										key={index}
										whileHover={{ y: -2 }}
				...
				<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className='mt-6'>
					<div className='bg-white dark:bg-gray-800 rounded-xl p-4 sm:p-6 shadow-sm border border-gray-100 dark:border-gray-700'>
						...
						<div className='grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3 sm:gap-4 mb-6'>
							{issueStatsCards.map((stat, index) => (
								<motion.div
									key={index}
									whileHover={{ y: -2 }}
						...
						<div className='overflow-x-auto max-w-[calc(100vw-2rem)] sm:max-w-full'>
							<table className='w-full'>
								<thead className='bg-gray-50 dark:bg-gray-700'>
									<tr>
										{['Type', 'Description', 'Reported By', 'Status', 'Action'].map(
											(header, index) => (
												<th
													key={index}
